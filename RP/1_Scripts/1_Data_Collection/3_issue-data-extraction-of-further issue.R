...
#path output
path_projectsInfo_output<-"../Projects-list/projectsInfo-additional-wontfix-data-RQ3.csv"
...
# TASK1: For each project with manually labeled wontfix issues we randomly  select at least 100 ???non-wontfix  issues???.

#path input

# path dataset containing the projects with manually labeled wontfix issues
path_projectsInfo<-"../Projects-list/projectsInfo-output_with_issue_urls-full-data-LABELED_categories_merged.csv"
...
#for(i in 1:2 )#length(projects_to_consider))
for(i in 33:length(projects_to_consider)) 
 {
        ...
        if(sum(str_detect(page,"O Closed"))==0){
          ...
          while(p<50 && end==0){
            ...
            if(sum(str_detect(page,"No results matched your search"))==0)
            {
              ...
              links_non_wontfix_issues_discussions <- page[which(str_detect(page,pattern))]
              links_non_wontfix_issues_discussions <- unique(str_extract(links_non_wontfix_issues_discussions,pattern))
              #to obtain the full links:
              links_non_wontfix_issues_discussions <- str_replace(links_non_wontfix_issues_discussions,"href=\"","https://github.com")
              links_non_wontfix_issues_discussions <- str_replace(links_non_wontfix_issues_discussions,"\"","")
...
i<-1
for(i in 3788:length(projectsInfo3$project_name)){
